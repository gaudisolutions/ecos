...
public class JavaAnalyzerService implements AnalyzerService{
				...
				if (modifiedPackageFiles.get(packageName) != null && actualPackageFiles.get(packageName) != null) {
					...
					for (String className : allClasses) {
						...
						if (modifiedLOCFiles.get(className) != null && actualLOCFiles.get(className) != null) {
							...
							if(actualLOCFiles.get(className).isMain()){
								
							changeLOCStructure.setVersion(searchVersion(actualLOCFiles.get(className)));
	...
	public boolean isVersionLine(String line) {
		String expression = String.format("\\*+\\<+VERSION+\\>+ \\w+.*");
	...
	/**
	 * This method searches the last version registered or starts a new version count if is the first time
	 * 
	 ...
	 * 
	 */
	private double searchVersion(LOCFile actualLOCFile) throws AnalyzerServiceException {
		...
		try {
			...
			for (int i = 0; i < locsActualFile.size(); i++) {
			
				if(locsActualFile.get(i)!=null){
				if (isVersionLine(locsActualFile.get(i).getLine())) {
					
					String vec[]=locsActualFile.get(i).getLine().split(" ");
					version=(Double.parseDouble(vec[1]))+1;
					return version;
			...
			throw new AnalyzerServiceException("Excepción al buscar la version" + actualLOCFile.getName(), e.getCause());
		}
	...
	private LOCFile compareLOCFiles(LOCFile modifiedLOCFile, LOCFile actualLOCFile) throws AnalyzerServiceException {
		...
		try {
			...
			for (int i = 0; i < locsModifiedFile.size(); i++) {
				...
				if (locsActualFile.get(i)!=null) {
				if (locsActualFile.get(i).isMain()){
					
					actualLOCFile.setMain(true);
		...
		locFile.setMain(modifiedLOCFile.isMain());
